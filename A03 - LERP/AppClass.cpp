...
void Application::InitVariables(void)
{
	...
	*/
	...
	{
		myVecs.push_back(GetCircle(0.95f + (i * 0.5f), 3 + i));
	}
}
...
void Application::Display(void)
{
	...
	*/
	...
	for (uint i = 0; i < m_uOrbits; ++i)
	{
		...
		vector3 lastPoint = myVecs[i][count % (3 + i)];
		vector3 nextPoint = myVecs[i][(count + 1) % (3 + i)];

		//calculate the current position
		vector3 v3CurrentPos = ZERO_V3;
		v3CurrentPos = ((nextPoint - lastPoint) * diff) + lastPoint;
...
std::vector<vector3> Application::GetCircle(float a_fRadius, int a_nSubdivisions)
{
	if (a_fRadius < 0.01f)
		a_fRadius = 0.01f;
	...
	std::vector<vector3> myVecs = std::vector<vector3>();
	/*
	...
	*/
	for (int i = 0; i < a_nSubdivisions; i++)
	{
		...
		myVecs.push_back(vector3(firstX, firstY, 0));
